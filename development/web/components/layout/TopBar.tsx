...
import { USE_MOCK } from "@/lib/kg/api";
import { ARTIFACT_LABEL, STATE_COLOR } from "@/lib/kg/graphTheme";
  ...
  return (
    <header className="pointer-events-none absolute inset-x-0 top-0 z-10 flex h-[78px] items-center gap-4 px-9">
      <div className="pointer-events-auto">
        <h1 className="text-[25px] font-medium leading-none tracking-[-0.045em] text-white">
          Knowledge
        </h1>
        <p className="mt-1.5 text-[12px] text-ink-dim">
          Your learning universe. Explore how concepts connect.
        </p>
      </div>
      ...
      {courses.length > 1 ? (
        <select
          ...
          className="pointer-events-auto h-8 max-w-[240px] rounded-md border border-white/10 bg-[#111722]/90 px-2 text-[12px] text-ink"
        >
          {courses.map((course) => (
      ...
      {USE_MOCK ? (
        <span className="pointer-events-auto rounded border border-white/10 px-1.5 py-[1px] text-[10px] font-medium text-ink-faint">
          Mock data
        </span>
      ) : null}
      ...
      <div
        ref={boxRef}
        className="pointer-events-auto relative ml-auto w-[330px]"
      >
        <Search
          size={13}

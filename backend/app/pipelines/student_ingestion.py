...
familiarity can move but mastery cannot.
...
Only this phase can move mastery. Concepts the student introduces that are
...
async def match_student_resource(
    ...
    *,
    ...
    existing = await get_resource_by_hash(session, course_id, content_hash, owner_user_id=student_id)
...
async def analyze_student_resource(
    ...
    for chunk_index, extraction in enumerate(extractions):
        ...
        for item_out in extraction.assessment_items:
            ...
            graded = item_out.score_achieved is not None and bool(item_out.max_score)

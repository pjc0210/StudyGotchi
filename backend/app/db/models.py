...
class StudentConceptState(Base):
    ...
    # Single learning-state measurement (spec: "single quantitative
    # learning-state measurement"). None = not enough evidence to estimate.
    # No familiarity/confidence/readiness/fragility columns: those are
    # either folded into this score or derived dynamically at query time
    # (app.domain.gaps.prerequisite_support), never persisted.
    understanding: Mapped[float | None] = mapped_column(Numeric, nullable=True)
    personal_relevance: Mapped[float | None] = mapped_column(Numeric, nullable=True)
    ...
    positive_evidence: Mapped[float] = mapped_column(
        Numeric, nullable=False, default=0.0
    )
    negative_evidence: Mapped[float] = mapped_column(
        Numeric, nullable=False, default=0.0
    )

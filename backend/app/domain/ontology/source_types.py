...
shaping canonical course structure vs. student understanding evidence.
"""
...
class AuthorityWeights:
    ...
    understanding_evidence: how much this source counts as understanding
        evidence (separate from the evidence-strength table in
        domain.mastery.evidence, which scales evidence_type; this scales the
        resource itself, e.g. an ungraded personal note vs. a graded exam of
        the same evidence_type).
    """

    curriculum_authority: float
    prerequisite_authority: float
    understanding_evidence: float
...
_DEFAULTS_BY_ORIGIN_ARTIFACT: dict[
    tuple[SourceOrigin, ArtifactType], AuthorityWeights
] = {
    (SourceOrigin.INSTRUCTOR, ArtifactType.LECTURE): AuthorityWeights(1.00, 0.95, 0.00),
    (SourceOrigin.INSTRUCTOR, ArtifactType.HOMEWORK): AuthorityWeights(
        1.00, 0.85, 0.00
    ),
    ...
    (SourceOrigin.INSTRUCTOR, ArtifactType.STUDY_GUIDE): AuthorityWeights(
        0.90, 0.85, 0.00
    ),
    (SourceOrigin.INSTRUCTOR, ArtifactType.SYLLABUS): AuthorityWeights(
        0.80, 0.60, 0.00
    ),
    (SourceOrigin.INSTRUCTOR, ArtifactType.SOLUTION_KEY): AuthorityWeights(
        0.95, 0.70, 0.00
    ),
    ...
    (SourceOrigin.STUDENT_SELF, ArtifactType.HOMEWORK): AuthorityWeights(
        0.40, 0.10, 0.80
    ),
    (SourceOrigin.STUDENT_SELF, ArtifactType.WORKED_SOLUTION): AuthorityWeights(
        0.40, 0.15, 0.65
    ),
    (SourceOrigin.STUDENT_SELF, ArtifactType.HANDWRITTEN_WORK): AuthorityWeights(
        0.35, 0.10, 0.55
    ),
    (SourceOrigin.STUDENT_SELF, ArtifactType.PHOTO): AuthorityWeights(0.35, 0.10, 0.55),
    (SourceOrigin.STUDENT_SELF, ArtifactType.STUDENT_NOTES): AuthorityWeights(
        0.35, 0.20, 0.15
    ),
    (SourceOrigin.CLASSMATE, ArtifactType.CLASSMATE_NOTES): AuthorityWeights(
        0.45, 0.35, 0.00
    ),
...
def get_authority(
    origin: SourceOrigin, artifact_type: ArtifactType
) -> AuthorityWeights:

"""Evidence events: the only source of truth for mastery (spec: "Student
evidence model"). File presence or resource links are never evidence by
themselves — only recorded events with an outcome/strength/certainty.
"""
...
# (notes, resource views) barely moves mastery.
...
_ASSESSMENT_TYPES = frozenset(
    {EvidenceType.GRADED_EXAM, EvidenceType.GRADED_QUIZ, EvidenceType.GRADED_HOMEWORK, EvidenceType.DIAGNOSTIC}
)
_PRACTICE_TYPES = frozenset({EvidenceType.VERIFIED_PRACTICE, EvidenceType.WORKED_SOLUTION, EvidenceType.SELF_EXPLANATION})
...
class EvidenceEvent:
    ...
    outcome: float | None  # 0.0 (incorrect) .. 1.0 (correct); None for pure-exposure events
    certainty: float  # how confidently this event is attributable to this concept, [0,1]
    occurred_at: datetime
    resource_id: UUID | None = None
    assessment_item_id: UUID | None = None
    difficulty: float = 1.0  # [0.5, 1.5], 1.0 = average
    concept_relevance: float = 1.0  # this concept's share of a multi-concept item, [0,1]
    strength: float | None = None  # overrides DEFAULT_EVIDENCE_STRENGTH[evidence_type] if set


def recency_weight(occurred_at: datetime, *, now: datetime | None = None, half_life_days: float) -> float:
...
def compute_evidence_weight(event: EvidenceEvent, *, now: datetime | None = None) -> float:
    """`weight_i = strength_i * certainty_i * difficulty_i * recency_i * concept_relevance_i`"""

    recency = recency_weight(event.occurred_at, now=now, half_life_days=evidence_half_life(event))
    return (

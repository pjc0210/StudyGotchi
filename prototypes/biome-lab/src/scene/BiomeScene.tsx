...
export function BiomeScene({ layout, state, onDiveEnd, onPickDistrict }: BiomeSceneProps) {
  ...
  const sky = night ? (world?.colours.skyNight ?? P.skyNight) : blizzard ? '#cfd6e2' : (world?.colours.sky ?? P.sky)
  ...
  return (
    <>
      ...
      {!world && (state.snowfall || blizzard || night) && <Snowfall density={blizzard ? 2.5 : night && !state.snowfall ? 0.15 : 0.4} layout={layout} />}

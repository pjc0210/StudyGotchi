...
interface PanelProps {
  layout: BiomeLayout
  /** Ice plus every layout found under /layouts (docs) or bundled as a lab draft. */
  biomes: Array<{ id: string; name: string; source: 'builtin' | 'docs' | 'lab-draft' }>
  ...
  return (
    <aside className="lab-panel">
      <header className="lab-header">
        ...
        <h1>{layout.name}</h1>
        <div className="lab-stations lab-biomes">
          {biomes.map((b) => (
            <button key={b.id} type="button" aria-pressed={layout.id === b.id} onClick={() => onPickBiome(b.id)} title={b.source === 'docs' ? 'docs/design/world/layouts' : b.source === 'lab-draft' ? 'lab-local draft layout' : 'built in'}>
              {b.name}
              {b.source === 'lab-draft' ? ' · draft' : ''}
            </button>
          ))}
        </div>
        <p>
          One course, one kingdom. Each district is a topic, shaped by the land, and grows by population: buildings, props,
          lights, residents. Only the skyline behind the town rises with the course.
          {layout.world ? ' Generic renderers (block kit): family-specific buildings, anchors and skylines are not yet realised.' : ''}
        </p>
      </header>

...
export function distToPolyline(points: Array<[number, number]>, x: number, z: number): { dist: number; t: number } {
...
export function pointAlong(points: Array<[number, number]>, s: number): { x: number; z: number; dx: number; dz: number } {
  let acc = 0
  for (let i = 1; i < points.length; i++) {
    const [ax, az] = points[i - 1]
    const [bx, bz] = points[i]
...
function segmentIntersection(a: [number, number], b: [number, number], c: [number, number], d: [number, number]): [number, number] | null {
...
export function polylineCrossings(a: Array<[number, number]>, b: Array<[number, number]>): Array<{ point: [number, number]; aDir: [number, number] }> {
  const out: Array<{ point: [number, number]; aDir: [number, number] }> = []
...
export type GroundMaterial = 'sea' | 'snow' | 'ice' | 'lake' | 'openWater' | 'rock' | 'mountain' | 'river' | 'riverBank'
...
export function sampleTerrain(layout: BiomeLayout, x: number, z: number, profile?: MountainProfile, peaks?: Peak[]): Sample {
  ...
  if (land > -2.5) {
    ...
    const slopeK = smoothstep(-2.5, layout.harbourShore.slopeLength - 2.5, land)
    const landK = cliffK * (1 - shore) + slopeK * shore
    const relief = layout.relief * 1.4 * fbm(x * 0.05, z * 0.05) * (1 - flat)
    let ground = layout.groundHeight + relief + layout.harbourShore.rise * smoothstep(2, layout.harbourShore.slopeLength, land) * harbourRiseMask(layout, x, z)
...
function computeHarbourFixtures(layout: BiomeLayout): HarbourFixtures {
  ...
  return {
    ...
    dock2Root,
    dock2Length: 15,
    icebreaker: [dockRoot[0] + dockDir[0] * ib + along[0] * 6.2, dockRoot[1] + dockDir[1] * ib + along[1] * 6.2],
...
export function districtSlots(layout: BiomeLayout, district: DistrictDef): Slot[] {
  ...
  if (district.id === 'forest' && district.shape.kind === 'strip') {
    ...
    const [cx, cz] = districtCentre(district)
    const ring = district.building === 'tent' ? 6.5 : district.building === 'quonset' ? 5.5 : 8.0
  ...
  for (const c of candidates) {
    ...
    if (district.id === 'station') {
      ...
      if (Math.hypot(c.x - hx, c.z - hz) < 5.5) continue
    }
    if (out.some((o) => Math.hypot(o.position[0] - c.x, o.position[2] - c.z) < 4.0)) continue
...
const RECIPES: Record<DistrictDef['id'], Recipe[]> = {
  harbour: [
  ...
  station: [
    { kind: 'fuel-drum', count: 4, minBand: 2, knockable: true },
    { kind: 'antenna', count: 1, minBand: 2 },
    { kind: 'crate', count: 3, minBand: 3, knockable: true },
    { kind: 'lamp', count: 2, minBand: 2 },
...
export function localPaths(layout: BiomeLayout): LocalPath[] {
  ...
  for (const c of layout.districts) {
    ...
    switch (c.id) {
      ...
      case 'station': {
        const [hx, hz] = helipadPoint(layout)
        out.push({ districtId: c.id, kind: 'lane', width: 1.2, minBand: 0, points: pathHeights(layout, [[cx - dx * 3, cz - dz * 3], [hx, hz]], 0.05, deck) })
        break
      }
    }
  }
...
export function residentSeats(layout: BiomeLayout, progress: DistrictProgress, count: number, seed: number): Seat[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (district.id === 'forest' || district.id === 'harbour') {

...
export interface BlobMotion {
  /** true while the creature is travelling; drives hop + squash */
  moving: boolean
  /** monotonically increasing phase in seconds */
  t: number
}
...
/**
 * Procedurally animated creature built from primitives: no rig, no clips.
 * Hop, squash and stretch, bob, blink, and an accessory chosen by variant.
 * Local +Z is the face. Feet at y = 0.
 */
export function Blob({ biome, variant, motion, onClick, selected }: BlobProps) {
  const body = useRef<THREE.Group>(null)
  const eyes = useRef<THREE.Group>(null)
  const pop = useRef(0)

  const bodyMat = useMemo(() => toonMaterial(biome.creature), [biome])
  const accentMat = useMemo(() => toonMaterial(biome.accent), [biome])
  const white = toonMaterial('#ffffff')
  const dark = toonMaterial('#2b2b33')
  const blush = toonMaterial('#f6a3a3')

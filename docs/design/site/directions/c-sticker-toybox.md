...
Game site for tone (one colour, one voice, deadpan copy, a few props in the margins). In one
line: a cream sheet with stamps on it. The page is paper, the world is the picture printed on
the paper, and everything else is a stamp (pixel mono, ink) or a sticker (a flat lozenge, one
colour). There are very few stickers.
...
every landmark and resident opens to the page it came from. The audience is an MIT
undergraduate who says courses by number (6.1210, 18.06), keeps problem sets in a folder she
never reopens, and will show a friend her world before she shows them her grades. The landing
page has one job: make her drop one course file, so a sprout appears where that idea lives.

## 2. Tokens

### Colour

Four named colours, one alpha ramp, one sticker family. The chrome owns no hue the world does
not already have, and it shows fewer at once than the world does.
...
70 (secondary text and every stamp). Perplexity's ramp ends at 65 and Megaminx Lab kept that; the
text step moves to 70 here because ink at 65 percent measures 4.3:1 on paper and stamps are 11 px
pixel type that needs the 4.5:1 body floor. At 70 percent it is 4.9:1.

Sticker family: the biome grounds from `.fleet/org.md`, unchanged. They fill biome chips and
nothing else: never text, never a page, never a card, never an outline.
...
Measured pairs: `ink` on `paper` 12.1:1, `paper` on `ink` 12.1:1, `ink` on `coral` 5.2:1.
`coral` text on `paper` is 2.3:1 and is forbidden; the golden HUD's coral "Demonstrated" word
becomes a coral-filled sticker with ink text. World `mint` and `light` are not chrome tokens.
...
Two weights on the whole site, plus a pixel face that has one.
...
Why Fredoka at 600. Nunito 800 is what the current landing already sets, so it is the habit, not
a choice, and with M PLUS Rounded 1c it is Direction A's rounded-gothic territory; Baloo 2 reads
as a nursery at any weight. Fredoka has round terminals that rhyme with the 999 px lozenges, wide
bowls that hold at 56 px next to the crunchy world, and a variable axis, so 600 can be set
...
Radius: 4 (kbd hints, swatch squares), 8 (inputs, secondary buttons), 12 (primary button, course
tiles), 16 (cards, the world window), 999 (state stickers, biome chips).
...
36 (primary button face; 40 with its edge, so the footprint sits on the 4 grid). Stamp to title 8.
Title to body 12. Card padding 16 at 390, 24 at 1440. HUD sheets 320 wide at 1440.

## 3. Layout

Concept: the world is printed on the sheet, every region of the sheet is stamped before it is
titled, sheets float over the world at 1440 and stack under it at 390, and nothing is outlined
except the three state stickers and the one button, so structure comes from stamps and whitespace.

### Landing, 1440

The hero is the thesis: the real 6.1210 world, awake, with its stamped card in the corner where
the golden HUD already puts it. Not a screenshot of the product; the product.
...
| and exam feedback become a   |
| world. Each thing in it ...  |
| [       Add a course       ] |
|   Look around 6.1210         |
| HOW THE GROUND FILLS IN      |
| +--------------------------+ |
...
At 390 the card leaves the world and sits under it, so the picture is never covered on a phone;
display drops to 40/44, the primary button goes full width, the props are gone.
...
coral on the screen besides the button. The biome chips under the card are lozenges in their
ground colour, no outline, only for biomes present in this world, largest land first.
...
+------------------------------+
| LECTURE 14 · PAGE 3          |  <- provenance sheet rises from
| Merge sort   (Demonstrated)  |     the bottom when a landmark
| [ Open the page ]     Close  |     or resident is tapped
+------------------------------+
```
...
strip above the world, carrying the one primary button on this screen, because the visitor is
the person we most want to add a course.
...
TOUCHED · SPROUT                      a rule card on the landing page
...
course name; they say pset 4 and L14 p.3. The evidence the product ingests is a stack of documents
that are themselves stamped with course, lecture and page numbers, and the world's whole promise
is that everything traces back to one of those stamps. So the chrome speaks the same way: the
...
Megaminx system that is a variant of Reveal, not a new pattern: the transform runs on
`--ease-spring` (overshoot to 1.043 at 40 percent, settled by 70), the opacity on
`--ease-out-quint` because the spring never eases an opacity, both over `--duration-tactile`
220 ms, from `scale(.92)` to none. Call it Reveal, sticker variant. "Pops" below means this.
...
| Aperture | Not used. The Iris is Megaminx Lab's. | |
...
fill changes. Focus is never announced by motion.
...
world"). Voice rules kept from the golden lines: active verbs, sentence case, the same verb
through a flow, errors say what to do next, deadpan and warm. Residents keep their lines exactly
as written in the golden spec. Stamps are the only capitals on the site.

## 7. Self-critique against defaults

The first draft was checked against the research doc's section 7 and against the frontend design
skill's three template looks. It drifted in five places.
...
1. Toward a full neubrutalist chrome. The first pass put the 2 px ink outline on the HUD sheets,
   ...
   flat paper with a 14 percent hairline. The world window has no border at all.
2. Toward rainbow chips. The first pass ran all seven biome chips across the top of every page as
   ...
   show the creature itself, and the only dot is the coral one that marks an arrival this
   session. `card-accent: 0` and `accent-cap: 1` exist so this drift is measurable next round.
...
4. Toward two soft faces. The first pass paired Fredoka with Nunito 500, both rounded, the kids'
   app read in one glance. Changed: Instrument Sans 500 for UI and body, Fredoka pulled from 700
   to 600 and barred under 28 px and from capitals.
5. Toward the cream template. Cream is mandated, so the check was whether the rest of the
   ...
   One catch on the way: the golden HUD's coral "Demonstrated" text fails contrast at 2.3:1, so
   this direction fixes it rather than inheriting it.

Also caught and removed: numbered markers on the three rule cards (only the state stickers
themselves may be numbered); a fourth margin prop; a footer line that named tools.
...
Source: ACNH dialogue, Game UI Database entry 606, via A's plan. Also the alpha state ramp as the
only source of greys, already adopted here from Megaminx Lab's Perplexity reading.

From Direction B, Modern DS: the two-screen frame at 390, already drawn into the world and visit
wireframes (world above, evidence below, a visible hinge; at 1440 the sheets float, on a phone
they should not cover the picture). Source: DSi menu via B's plan. Also the flat tile grid for
the course selector: one paper tile per course, each stamped with its number and biome, no
thumbnail chrome, 12 px radius. Source: Wii channel grid read through Frutiger Metro, via B's plan.

Not borrowed from either: lilac grey as the page (the world already floats on it inside the
canvas, and cream around sky is the whole point of the sheet), and any humanist display cut (the
chunk is this direction's one aesthetic risk, and it is spent on Fredoka).

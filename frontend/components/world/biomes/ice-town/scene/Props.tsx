 ...
 * No cargo GLB exists in assets — shared-rowboat is a dinghy, not a freighter.
 */
export function Boat() {
  ...
  return (
    <group name="ice-cargo-ship" position={[0, 0.08, 0]}>
      ...
      {crates.map((crate, i) => (
        <mesh key={i} position={crate.pos} castShadow>
          <boxGeometry args={[0.95, 0.48, 0.82]} />
          <Surf color={crate.color} />
        </mesh>
      ))}
      <mesh position={[0, 1.35, -1.55]} castShadow>
        <boxGeometry args={[1.7, 0.95, 1.35]} />
        <Surf color={P.cream} />
      </mesh>
      ...
      {[-0.38, 0.38].map((x) => (
        <mesh key={x} position={[x, 2.08, -1.26]}>
          <boxGeometry args={[0.28, 0.22, 0.04]} />
          <Surf color={P.light} />
        </mesh>
      ))}
      <mesh position={[0, 2.55, -1.95]} castShadow>
        <cylinderGeometry args={[0.22, 0.26, 0.7, 8]} />
        <Surf color={P.ink} />
      </mesh>
      <mesh position={[0, 2.88, -1.95]}>
    ...
    <group>
      {planks.map((p, i) => (
        <mesh key={i} position={[p.x, p.y, 0]} rotation={[0, 0, p.tilt]} castShadow>
          <boxGeometry args={[length / n + 0.04, 0.16, 2.4]} />
          <Surf color={i % 2 ? P.planks : P.planksLight} />
        </mesh>
      ))}
      {[-1.15, 1.15].map((z) =>
        [0.08, 0.3, 0.5, 0.7, 0.92].map((t) => (
          <Post key={`${z}:${t}`} position={[(t - 0.5) * length, 0.35 + Math.sin(t * Math.PI) * 0.55 + 0.5, z]} h={1.0} r={0.06} />
        )),
      )}
      {[-1.15, 1.15].map((z) => (
        <mesh key={z} position={[0, 1.28, z]} castShadow>
          <boxGeometry args={[length * 0.9, 0.12, 0.12]} />
          <Surf color={P.timberDark} />
        </mesh>
      ))}
    </group>
      ...
      {Array.from({ length: ribs }, (_, i) => {
        ...
        return (
          <mesh key={i} position={[-3.6 + t * 5.6, spineY - 0.1, 0]} rotation={[0, Math.PI / 2, Math.PI]} castShadow>
            <torusGeometry args={[Math.max(0.6, r), 0.16, 5, 9, Math.PI]} />
            <Surf color={P.bone} />
          </mesh>
        )
      })}
...
export function Icebreaker({ lit }: { lit: boolean }) {
  return (
    <group position={[0, -0.2, 0]}>
      ...
      {[
        { x: -1.15, z: 2.4, color: P.peach },
        { x: 1.15, z: 2.4, color: P.timberTeal },
        { x: -1.15, z: 3.7, color: P.coral },
        { x: 1.15, z: 3.7, color: P.lilac },
        { x: 0, z: 3.05, y: 3.55, color: P.timberRed },
      ].map((crate, i) => (
        <mesh key={`c${i}`} position={[crate.x, crate.y ?? 3.15, crate.z]} castShadow>
          <boxGeometry args={[2.0, 1.0, 1.15]} />
          <Surf color={crate.color} />
        </mesh>
      ))}
    </group>

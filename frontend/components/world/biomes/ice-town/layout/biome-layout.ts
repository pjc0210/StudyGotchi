...
/** The Ice / Chilly Town district ids; layouts loaded from JSON use their own ids. */
export type IceDistrictKind = 'harbour' | 'town' | 'lake' | 'forest' | 'glacier' | 'station'
export type DistrictKind = IceDistrictKind | (string & {})

/** The Ice building families; layouts loaded from JSON name their own (resolved by the world families). */
export type IceBuildingKind = 'stilt-house' | 'timber-house' | 'fishing-hut' | 'forest-house' | 'tent' | 'quonset'
export type BuildingKind = IceBuildingKind | (string & {})

export type DistrictMaterial = 'snow' | 'planks' | 'ice' | (string & {})

/* ---------- generic world model (layouts loaded from docs/design/world/layouts) ---------- */

export type SkylineKind = 'mountain-range' | 'gantry-wall' | 'spire-rows' | 'hall-skyline' | 'candy-peaks'

/**
 * One growth band of the skyline, normalised across kinds: `count` primary silhouettes at `height`
 * (peaks / gantries / halls / spires / whipped peaks), a `secondary` class (chimneys / towers /
 * lollipops), a `tertiary` class (cooling towers / domes / clouds), one `hero` (bell tower /
 * tallest spire), `rows` deep into the fog, `lit` bands or windows at night.
 */
...
export interface SkylineSpec {
  kind: SkylineKind
  /** The row follows z = arcZ + arcCurve·x². */
  arcZ: number
  arcCurve: number
  /** Where the settled land ends and the skyline plinth begins. */
  footZ: number
  fogStartZ: number
  fogOpaqueZ: number
  /** Spacing of the primary silhouettes along x. */
  pitch: number
  /** Five bands: 0, 25, 50, 75, 100 %. */
  growth: [SkylineBand, SkylineBand, SkylineBand, SkylineBand, SkylineBand]
  /** Hero element position (bell tower, tallest spire), if the layout fixes one. */
  heroPosition?: [number, number]
}

export type WaterKind = 'sea' | 'lake' | 'river' | 'canal' | 'basin' | 'lagoon' | 'pool' | 'pond'

export interface WaterBody {
  id: string
  kind: WaterKind
  /** Either a polygon or a centreline + width. */
  polygon?: Array<[number, number]>
  centreline?: Array<[number, number]>
  width?: number
  /** Ring pools: the inner disc stays dry. */
  innerRadius?: number
  colour?: string
  /** Inland bodies cut a channel into the land; `false` means the body is the open sea. */
  inland: boolean
}

export type TransitKind = 'monorail-loop' | 'rail-spine' | 'causeway' | 'road' | 'pipe-rack' | 'conveyor' | 'path'
...
export interface TransitLine {
  ...
  closed: boolean
  /** Deck height above the ground (0 for ground-level lines). */
  height: number
  width: number
  stations?: Array<{ at: [number, number]; name: string; district?: string }>
  /** Population band at which the line appears (0 = always). */
  minBand: number
  colour?: string
  /** Pods / trains on the line. */
  vehicles?: { count: number; speed: number; dwell: number; body: string; nose: string }
  /** Seeded schedule (rail spine trains): every [min, max] seconds, visible for `visible` seconds. */
  event?: { every: [number, number]; visible: number }
}

export type LandmarkMotif = 'dome' | 'gear' | 'spire' | 'castle' | 'lighthouse'

export interface LandmarkSpec {
  position: [number, number]
  plinthRadius: number
  plinthHeight: number
  /** Plinth footprint if the layout gives a polygon (slab) instead of a disc. */
  plinthPolygon?: Array<[number, number]>
  causeway?: Array<[number, number]>
  motif: LandmarkMotif
  stages: Array<{ id: string; name: string; height: number; footprint: [number, number] }>
}

export interface SoloIsland {
  id: string
  center: [number, number]
  radius: number
  polygon: Array<[number, number]>
  /** District that owns the island (its anchor is the island feature). */
  districtId?: string
}

/** Palette roles every generic renderer can rely on; resolved from the layout's raw palette. */
...
export type WorldFamily = 'academy' | 'industry' | 'utopia' | 'candy' | 'generic'

export type CoastStyle = 'cliff' | 'quay' | 'glass' | 'plush'

export interface WorldSpec {
  family: WorldFamily
  /** Raw palette from the layout file. */
  palette: Record<string, string>
  colours: WorldColours
  /** Mainland outline; the land continues for z < backZ into the fog. */
  mainland: Array<[number, number]>
  backZ: number
  coast: CoastStyle
  cliffDrop: number
  /** Coast wobble in metres (0 for engineered quays and glass risers). */
  coastWobble: number
  water: WaterBody[]
  transit: TransitLine[]
  skyline: SkylineSpec
  landmark: LandmarkSpec
  soloIsland?: SoloIsland
  /** Ground-colour patches: lawns, plazas, slag shores. */
  patches: Array<{ id: string; polygon: Array<[number, number]>; material: 'lawn' | 'paving' | 'shore' | 'groundAlt' }>
  /** Sea creature event (whale / manta) reusing the whale schedule: arc + colours. */
  creatureEvent?: { arc: { center: [number, number]; radius: number }; body: string; belly: string; kind: 'whale' | 'manta' }
  /** Ambient boats on a seeded ellipse loop. */
  boats?: { center: [number, number]; rx: number; rz: number; count: number; sail: string; hull: string; speed: number }
}

/** How a district's anchor is built; inferred from the layout's `anchor` text by the adapter. */
...
export interface DistrictDef {
  ...
  /** District-station pitch and fit-width when the layout's `camera` sets them. */
  stationPitch?: number
  stationFit?: number
  ...
  /* ---- generic world districts (from layout JSON) ---- */
  /** Building height in metres (the family scales its kit to it). */
  buildingHeight?: number
  /** Feature list from the layout, for the panel and the prop recipes. */
  features?: string[]
  /** How the anchor is built and where; `anchorPosition` defaults to the centroid. */
  anchorKind?: AnchorKind
  anchorPosition?: [number, number]
  /** Solitary island district. */
  solitary?: boolean
  /** Extras that appear at 100 % (free text from the layout). */
  extrasAt100?: string
}
...
export interface BiomeLayout {
  ...
  /**
   * Generic world description for layouts loaded from JSON. When present the Ice-specific fields
   * above are filled with neutral values and every renderer takes the generic path.
   */
  world?: WorldSpec
}
...
export const ICE_TOWN_LAYOUT: BiomeLayout = {
  ...
  miniIsland: { center: [78, 68], radius: 14.5 },
  headland: { center: [53, 48], radius: 5.5 },
  whaleArc: { center: [14, 84], radius: 9 },
  harbourShore: { slopeLength: 20, rise: 1.8 },
  forestMargin: 24,
  creatureHeight: 1.8,
  creatureScale: 1.65,
  districts: [
    {
      ...
      laneYaw: 1.5,
      // Over the water from the south-east (fishing-village ref): the stilt row runs diagonally
      // across the frame, the land rises behind it. Straight from the sea (0°) the chord grows to
      // 58 m and the creature drops under the 6 % rule; 72° keeps 6.2 %.
      stationAzimuth: 72,
    ...
    },
    {
      ...
      name: 'Mountain camp',
      band: 5,
      anchor: 'ice arch · approaches · cave',
...
/** The skyline band (0–4) for a layout's growth table at a course fraction. */
export function skylineBandFor(layout: BiomeLayout, fraction: number): SkylineBand | null {
  if (!layout.world) return null
  return layout.world.skyline.growth[bandForProgress(fraction)]
}
...
export interface LayoutExport {
  ...
  /** Generic layouts: the active skyline band. */
  skyline?: { kind: SkylineKind; band: SkylineBand }
...
export function buildLayoutExport(layout: BiomeLayout, progress: DistrictProgress, camera: unknown): LayoutExport {
  ...
  const sky = skylineBandFor(layout, fraction)
  return {
    version: 3,
    biome: layout.id,
    ...(layout.world && sky ? { skyline: { kind: layout.world.skyline.kind, band: sky } } : {}),
